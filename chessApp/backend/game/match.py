from chessApp.backend.game.board import ChessBoard
from chessApp.backend.game.pieces import Pawn, Rook, Knight, Bishop, Queen, King
from chessApp.backend.game.move import Move

from chessApp.backend.game.helper_functions import (translate_from_notation,
...
class Match():
            ...
            if move.castling:
                ...
                if move.taking_piece:
                    try:
                        ...
                        print('MOVE FAILED: ', move.piece.display())
                        for piece in self.pieces[move.piece.color]:
                            print(piece.display(), piece.position)
                piece.move(move)
    ...
    def make_a_move(self, move, needing_checkmate_flag=True,
        ...
        move.piece = self.chessboard.return_piece_on_field(move.start_pos)
    ...
    def revert_move(self, move):
        """Returning to a state before a move"""

        # revert move
        move.piece.move(move, reverse=True)
        if move.taking_piece:
            move.taking_piece.move(move)
            if move.en_passant:
                # adjusting piece.position if en_passant
        ...
        if move.promotion:
            # revert promotion, taking new piece of list and board, pawn back on
            self.pieces[move.piece.color].remove(move.piece)
            pawn = Pawn(move.start_pos, move.piece.color, move.piece.chessboard)
            self.chessboard.place_on_board(pawn)
            self.pieces[move.piece.color].append(pawn)
            move.piece = pawn

        move.piece.en_passantable = False
        move.piece.move_count -= 1
        move = Move(move.start_pos, move.end_pos, move.chessboard)
        self.change_turns()
